...
static PyObject * #파이썬 객체를 c 데이터타입으로구현
float SeoulAverSaves=0.0f; #전역변수
...
static PyMethodDef SpamMethods[] = {//스펨메소드
    {"GetAllValue", spam_SeoulAver, METH_VARARGS,//스트링렝뜨 와 디비전이있다고 기술
    "서울시의 평균 대기 상태를 계산을 위해 각 구의 값을 받아옴"},
    {"SetSeoulAver", spam_SeoulResultReturn, METH_VARARGS,//스트링렝뜨 와 디비전이있다고 기술
    "25개 구의 값을 전부 처리하면 이중 유효한 구(점검중이 아닌 구)가 몇 개인지 받아와서 평균값을 계산함.},
...
{
    float* seoulF=0.0f;//인자값 저장할 임시변수
    index++;//전역 인덱스 증가
...
spam_SeoulResultReturn(PyObject *self, PyObject *args){
    ...
    return float(SeoulAverSaves/index)
